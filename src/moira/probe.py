...
def deduplicate_probe_structures(
    probe_atoms_list: list[Atoms],
    dedup_atom_indices_list: list[tuple[int, ...]],
    adaptor: Any,
    matcher: Any,
    slab_atom_count: int,
) -> list[Atoms]:
...
def probe_match_signature(structure: Any) -> tuple[str, int]:
...
def _dataset_path_from_config(cfg: Config) -> Path:
...
def write_probe_artifacts(
    *,
    ...
    try:
        ...
        raise ImportError(
            "Probe dataset generation requires pymatgen. Install pymatgen to use moira.probe."
        ) from exc
