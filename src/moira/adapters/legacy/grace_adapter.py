# Legacy GRACE adapter using GRACEMaker's TensorFlow ASE calculator.
...
DEFAULT_MLIP_NAME = "grace"


def _resolve_required_model_path(
    path_value: str,
    *,
    config_path: str,
    field_name: str,
...
def run(
    *,
    ...
    del device  # TPCalculator selects TensorFlow devices from the environment.
    ...
    resolved_checkpoint = _resolve_required_model_path(
        checkpoint,
        config_path=config_path,
    ...
    calculators = [TPCalculator(resolved_checkpoint) for _ in range(n_calcs)]
